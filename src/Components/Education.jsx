...
const EducationCard = ({ education }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-secondary/20 rounded-lg border border-white/6 border-t-2 border-t-accent/30 overflow-hidden cursor-pointer transition-colors hover:bg-secondary/35 ${isExpanded ? 'ring-1 ring-accent/30' : ''}`}
    >
      <div className="p-6">
        <div className="flex justify-between items-start gap-4">
          <div className="flex-grow">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden bg-white p-1.5 flex-shrink-0">
                <img src={logo} alt={education.institution} className="w-full h-full object-contain" />
              </div>
              <h3 className="text-xl font-heading font-bold text-text">{education.degree}</h3>
            </div>

            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-text-muted mb-3">
              <span className="font-medium text-accent/90">{education.institution}</span>
              <span>•</span>
